	<div id="content" class="col-lg-10 col-sm-10">
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$('#confirmWarishGenarate').submit(function() {
					$("#load").empty().append('<img src="library/img/ajaxloader.gif">');
					$.post(
					"Genarate/warishGenarate_action",
					$("#confirmWarishGenarate").serialize(),
					function(data){
						if(data==1)
						{
							document.getElementById('save_tlicence').type='button';
							$("#globalmsg").show("normal").html('<p class="done">আপনার ওয়ারিশ সনদ সঠিকভাবে তৈরি হয়েছে,আপনি মানি রিসিট গ্রহণ করুন</p>');
							setTimeout(function() {
							window.location='Applicant/warishapplicant?napply=5';}, 2000)
						}
		...
		<div class="row">
			<div class="col-lg-12 col-sm-12 col-xs-12"> 
				<button class="btn btn-custom-head btn-block btn-sm" style="font-size:16px;margin-bottom:30px;">Confirm ওয়ারিশ আবেদন জেনারেট </button>
			</div>
				<!-- Form -->
			<div class="col-lg-9 col-sm-9 col-lg-offset-3 col-sm-offset-3"> 
				<form action="Genarate/warishGenarate_action" method="post" id="confirmWarishGenarate" enctype="multipart/form-data">
					...
					<div class="form-group"> 
						<label for="Verification" class="control-label col-sm-3 col-xs-6 big-font">তদন্তকারীর নাম :</label>
						<div class="col-sm-9 col-xs-6"> 
							<span class="most_highlight"><?php echo $verifier_name; ?></span>
						</div>
						<div class="clearfix"></div>
					</div>
					<div class="form-group"> 
					...
					<div class="form-group"> 
						<div class="col-sm-10 col-sm-offset-2">
							...
							<input type="hidden" name="save_tlicence" value="continue"/>
							<input type="hidden" name='trackid' value='<?php echo $trackid;?>'/>
							<input type="hidden" name="uid" value="<?php echo $uid;?>"/>
							<input type='hidden' name='apno' value='<?php echo $apno;?>'/>
							<input type='hidden' name='bfname' value='<?php echo $bfname;?>'/>
							<input type='hidden' name='verifier_name' value='<?php echo $verifier_name;?>'/>
