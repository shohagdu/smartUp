	...
	<?php
		...
		extract($_GET);
		if(trim($ac!="")){$account="sub_title='$ac' AND";}
		if(trim($to)!="" && trim($from)!=""){
		$range="date(payment_date) between '$to' and '$from'";}
		$query=$this->db->query("select * from fund_sub_ctg where $account $range order by voucherno ASC");
		$result=$query->result();
	?>
	
	<div style="padding:4px;width:100%;font-family:tahoma;" class="table-responsive">
		<table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%" border="1px">
