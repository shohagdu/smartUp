			...
			<div id="main_second" <?php if($muri==0):echo $main_second2;endif;?>>
				...
				<div id="muri"> 
						...
						<tr>
							...
							<td <?php if($header==0) { ?> class="no-print" <?php } ?> style="text-align:center;">
								<div class="headding-main"> 
									<font style="font-size: 96%; font-weight:bold; color:blue;"><?php echo (!empty($all_data->full_name) ? $all_data->full_name : '');?></font>  <br />
					...
					<table width="95%" height="186px" border='0px' class="muri_table" style="border-collapse:collapse;margin: 0px auto;table-layout: fixed;"> 
						...
						<tr>
							...
							<td colspan="3"><p> : <?php echo (!empty($row->bb_gram) ? $row->bb_gram. ', ' : '').(!empty($row->bb_rbs) ? $row->bb_rbs. ', ' : '').(!empty($row->bb_wordno) ? $row->bb_wordno.', ' : '').(!empty($row->bb_postof) ? $row->bb_postof. ', ' : '').(!empty($row->bb_thana) ? $row->bb_thana. ', ' : '').(!empty($row->bb_dis) ? $row->bb_dis : '');?></p></td>
						</tr>
						<tr>
							<td style="font-size:80%;vertical-align:middle;">ব্যবসার ধরন</td>
							<td colspan="3"><p> : <?php echo (!empty($row->btype) ? $row->btype : '');?></p></td>
						</tr>
						<tr>
				...
				<div class="wrapper jolchap" <?php if($muri==0):echo $wrapper2;endif;?>>
					<div id="cert" <?php if($muri==0):echo $cert2;endif;?>>
						...
						<table width="99%" height="35px" border="0" style="border-collapse:collapse;margin:0px auto; margin-top: 20px;" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td>
									<table border='1' width="60%" height="28px" cellpadding='0' cellspacing='0' style="border-collapse:collapse; table-layout:fixed;"> 
										<tr> 
											...
											<?php 
												$trade_license_no=substr($row->sno,-5);
						...
						<table width="97%" height="222px" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse; margin: 0px auto; ">
							...
							<tr style="height: 62px;">
								...
								<td valign='top' style="font-weight:bold; font-size:18px; text-align:left;">:&nbsp;<?php echo (!empty($row->bb_gram) ? $row->bb_gram. ', ' : '').(!empty($row->bb_rbs) ? $row->bb_rbs. ', ' : '').(!empty($row->bb_postof) ? $row->bb_postof. ', ' : '').(!empty($row->bb_thana) ? $row->bb_thana. ', ' : '').(!empty($row->bb_dis) ? $row->bb_dis : '');?><?php echo (!empty($row->bb_wordno) ? '| ওয়ার্ড নং - '.$row->bb_wordno : '');?>
								</td>
							</tr>
							<tr> 
								<td style="text-indent: 20px; text-align:left; font-size:17px;">ব্যবসার ধরন</td>
								<td  style="font-weight:bold; font-size:18px; text-align:left;">:&nbsp;<?php echo (!empty($row->btype) ? $row->btype : '');?></td>
							</tr>
							<?php if($row->mobile !=''):?>
							<tr>
