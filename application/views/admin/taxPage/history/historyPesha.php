<?php 
	...
	$rTrade=$this->db->query("select * from money where  trackid='$trackid' and status='3'")->result();
	if($this->db->affected_rows()<=0){
		/*echo "<p style='padding-left:500px;color:red;'>আপনার ট্রেড লাইসেন্স নং টা সঠিক নয়</p>";exit;*/
	}
	else{
?>
		<button class="btn btn-primary btn-block"> পেশাজীবি কর  এর Last Paymand History</button>
		<table class="table table-striped table-bordered dt-responsive nowrap">
			...
			?>
					<tr>
						...
						<td style="text-align:center;"><a href="Money_receipt/peshaMoneyReceipt?id=<?php echo $row->id?>&vno=<?php echo $row->inno?>&sno=<?php echo $row->trackid?>" class='btn btn-success btn-sm' target='_blank'>প্রিন্ট</a></td>
